...
let SignalingServer = class SignalingServer {
    reload(cert) {
    ...
    start(cfg) {
        ...
        this._server = tls.createServer(options, (socket) => {
            this._signalingConnection.takeoverSocket(socket, false);
        });
        this._server.listen(cfg.port, () => {
            this._logs.info({
                'action': 'SignalingServer',
                'message': 'start ok',
                'data': {
                    'port': cfg.port
                }
...
], SignalingServer.prototype, "_signalingConnection", void 0);
__decorate([
    Logs.UseLogger('default'),
    __metadata("design:type", Object)
], SignalingServer.prototype, "_logs", void 0);
SignalingServer = __decorate([
    DI.Singleton()
], SignalingServer);
