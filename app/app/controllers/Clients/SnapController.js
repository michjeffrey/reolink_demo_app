...
class SnapController {
    async getDeviceSnap(ctx) {
        const uid = ctx.request.params['uid'];
        const streamType = ctx.request.query['stream_type'];
        const channel = ctx.request.query['channel'];
        try {
            const result = await this._snapSession.getSnap({
                'channel': parseInt(channel),
                streamType,
                uid
            });
            ctx.response.send(Buffer.concat(result));
        }
        catch {
            throw new client_1.E_SNAP_ERROR();
        }
    }
}
__decorate([
...
], SnapController.prototype, "_snapSession", void 0);
__decorate([
    Http.Get('/v1.0/devices/{uid:string}/snap/snapshot.jpg', {
        'validator': {
            'params': tgc.compile({
                'rule': {
...
], SnapController.prototype, "getDeviceSnap", null);
exports.default = SnapController;
